...
class Main(webdriver.Chrome):
    ...
    def initalize_database(self):
        create_table_races_query = """CREATE TABLE races (id INT NOT NULL AUTO_INCREMENT, pos VARCHAR(255),  no VARCHAR(255), driver VARCHAR(255), car VARCHAR(255), laps VARCHAR(255), retired VARCHAR(255), pts INT, date VARCHAR(255), location VARCHAR(255), location_key VARCHAR(255), year VARCHAR(255), PRIMARY KEY (id));"""
        self.check_exists_table('races', create_table_races_query)
            ...
            for year_tuple in year_list:
                ...
                if len(category_list) > 0:
                    for category_tuple in category_list:
                        ...
                        if category_key in [RACES, DRIVERS, TEAMS]:
    ...
    def crawl_result_racing(self, category_key, link_key, detail_value, year_value):
        if category_key == RACES:
            ...
            pass
        elif category_key == TEAMS:
            pass
    
    def crawl_result_racing_with_races_table(self):
        drivers_df =  copy.deepcopy(define.races_df) 
        html = self.page_source
        soup = BeautifulSoup(html, "html.parser")
    ...
    def upsert_races_into_database(self, df):
        for index, row in df.iterrows():
            ...
            check_row_exists_query = f"SELECT id FROM races WHERE driver = '{driver}' AND location_key = '{location_key}' AND year = '{year}'"
            db_data = pd.read_sql(check_row_exists_query, con=self.engine)
            
            if len(db_data) > 0:
                id = db_data['id'].values[0]
                update_query = f"UPDATE races SET pos = '{pos}', no = '{no}', car = '{car}', laps = '{laps}', retired = '{retired}', pts = '{pts}', date = '{date}' WHERE id = {id}" 

                self.cursor.execute(update_query)
            else:
                insert_query = f"INSERT INTO races (pos, no, car, laps, retired, pts, date, driver, location, location_key, year) VALUES ('{pos}', '{no}', '{car}', '{laps}', '{retired}', '{pts}', '{date}', '{driver}', '{location}', '{location_key}', '{year}')"
